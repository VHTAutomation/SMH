...
Mobile.tap(findTestObject('ObjectThemGuongMat/txt_dich_vu_camera (1)'), 0)
...
if (Mobile.waitForElementPresent(groupNguoiNha, 15, FailureHandling.CONTINUE_ON_FAILURE)) {
    ...
    if (Mobile.waitForElementPresent(face, 10, FailureHandling.CONTINUE_ON_FAILURE)) {
        println('Gương mặt \'Thu Giang5\' tồn tại trong nhóm \'Gia đình\'.')
    } else {
        println('Gương mặt \'Thu Giang5\' không tồn tại trong nhóm \'Gia đình\'.')
    }
} else {
